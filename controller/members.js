...
exports.UpdateMemberRole = async (data) => {
  const { token, userID, memberID, role } = data;
  try {
...
exports.RemoveMemberFromGroup = async (data) => {
  const { token, userID, memberID, groupID } = data;
  try {
    const isAuthenticated = await authenticateByTokenAndUserID(
      token,
      userID
    ...
    if (token && isAuthenticated) {
      ...
      if (UserData) {
        ...
        try {
          ...
          return {
            message: "Error fetching or merging members:",
            status: "error",
          };
        }
      } else {
...
exports.AddMembersToGroup = async (data) => {
  const { token, userID, memberID, groupID, ids } = data;
  try {
    const isAuthenticated = await authenticateByTokenAndUserID(
      token,
      userID
